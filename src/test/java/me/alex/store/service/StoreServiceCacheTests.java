...
class StoreServiceCacheTests extends AbstractContainerTest {
    ...
    void both_caches_work() {
        ...
        assertThat(caffeineCacheManager.getCache("storeCache").get(user.getUsername()).get()).isEqualTo(memCacheHit);
        var redisCacheHit = (StoreDto[]) redisCacheManager.getCache("storeCache").get(user.getUsername()).get();
